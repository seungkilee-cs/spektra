...
export async function computeSpectogram(file, fftSize = 1024, overlap = 0.5) {
  ...
  // AudioContext with browser compatibility
  ...
  // Decode audio
  console.time("Audio decode");
  const audioBuffer = await audioContext.decodeAudioData(arrayBuffer);
  console.timeEnd("Audio decode");
  ...
  console.log(`Processing ${numWindows} windows with FFT size ${fftSize}`);

  const spectogramData = [];

  // Process each window
  console.time("FFT Processing");
  for (let windowIndex = 0; windowIndex < numWindows; windowIndex++) {
    const startIndex = windowIndex * hopSize;

    // Extract window
    const windowData = new Array(fftSize).fill(0);
    for (let i = 0; i < fftSize && startIndex + i < audioData.length; i++) {
    ...
    const magnitudes = new Array(Math.floor(fftSize / 2));
    ...
    spectogramData.push(magnitudes);

    // Progress logging (every 10% of windows)
    if (windowIndex % Math.floor(numWindows / 10) === 0) {
      console.log(`Processed ${windowIndex}/${numWindows} windows`);
    }
  }
  console.timeEnd("FFT Processing");

  console.log(
    `Generated spectrogram: ${spectogramData.length} x ${spectogramData[0].length}`,
  );
